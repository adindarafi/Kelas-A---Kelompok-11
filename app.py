...
class AplikasiSiswa:
    def __init__(self, root):
        ...
        try:
            self.bg_image_login = ImageTk.PhotoImage(Image.open("background login.jpg").resize((1400, 700)))
            self.bg_image_input_id = ImageTk.PhotoImage(Image.open("background id.jpg").resize((1400, 700)))
            self.bg_image_input_data = ImageTk.PhotoImage(Image.open("background data siswa.jpg").resize((1400, 700)))
            self.bg_image_tabel_data = ImageTk.PhotoImage(Image.open("background tabel.jpg").resize((1400, 700)))
        ...
        tk.Button(self.frame_input_id, text="Kembali", font=("times new roman", 16), bg="beige", command=self.kembali_login).place(relx=0.5, rely=0.55, anchor="center")
...
# Frame Input Data Siswa
    ...
    def buat_frame_dengan_bg(self, parent, bg_image):
            """
            Fungsi untuk membuat frame dengan latar belakang gambar.
            """
            frame = tk.Frame(parent)
            canvas = tk.Canvas(frame, width=1920, height=1080)
            canvas.pack(fill="both", expand=True)
            canvas.create_image(0, 0, image=bg_image, anchor="nw")
            frame.canvas = canvas  # Simpan referensi agar gambar tidak hilang
            return frame
