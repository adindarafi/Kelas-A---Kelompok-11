...
from utils import baca_data_siswa, simpan_data  # Fungsi untuk membaca dan menyimpan data siswa

# Nama file default
file_siswa = "D:\\projecta\\Kelas-A---Kelompok-11\\modul\\data_siswa.xlsx"
...
class AplikasiSiswa:
    def __init__(self, root):
        ...
        try:
            self.bg_image_login = ImageTk.PhotoImage(Image.open("background login.jpg").resize((1280, 725)))
            self.bg_image_input_id = ImageTk.PhotoImage(Image.open("background id.jpg").resize((1280, 725)))
            self.bg_image_input_data = ImageTk.PhotoImage(Image.open("background data siswa.jpg").resize((1280, 725)))
            self.bg_image_tabel_data = ImageTk.PhotoImage(Image.open("background tabel.jpg").resize((1280, 725)))
       ...
       # Frame Input ID
        ...
        tk.Button(
        self.frame_input_id,
        text="Lihat Data Siswa",
        font=("times new roman", 16),
        bg="beige",
        command=lambda: self.tampilkan_data_siswa(baca_data_siswa())
        ...
        # Tambahkan bind untuk edit pada Treeview
    ...
    def tampilkan_data_siswa(self, data_siswa=None):
        """Tampilkan data siswa pada tabel. Muat ulang dari file jika tidak ada data yang diberikan."""
        self.frame_input_id.pack_forget()
        ...
        # Muat data siswa dari file jika tidak diberikan
    ...
    def logout(self):
        ...
        self.frame_tabel_siswa.pack_forget()
        self.tampilkan_data_siswa()  # Reload data saat logout
        self.frame_login.pack(fill="both", expand=True)

        
    def edit_cell(self, event):
        """Memungkinkan pengguna untuk mengedit sel pada tabel dengan label dan entry sejajar."""
        # Mendapatkan item dan kolom yang diklik
        ...
        column_index = int(column[1:]) - 1  # Konversi kolom Treeview ke indeks
        bbox = self.tree.bbox(item_id, column)  # Dapatkan bounding box sel
        if not bbox:
            return
        ...
        # Membuat Entry untuk mengedit data
        entry_edit = tk.Entry(self.frame_tabel_siswa, font=("times new roman", 12))
        entry_edit.insert(0, current_value)
        entry_edit.place(relx=0.5, rely=0.2, anchor="center", width=bbox[2], height=bbox[3])  # Posisi sesuai bounding box
        entry_edit.focus()
        ...
        def save_edit():
            ...
            # Update DataFrame dan Simpan ke File Excel
            data_siswa = baca_data_siswa()
            row_index = self.tree.index(item_id)
            data_siswa.iloc[row_index, column_index] = new_value
            simpan_data(file_siswa, data_siswa)

            messagebox.showinfo("Sukses", "Data berhasil diperbarui!")

        # Ketika pengguna selesai mengedit
        entry_edit.bind("<Return>", lambda event: save_edit())
        entry_edit.bind("<FocusOut>", lambda event: (entry_edit.destroy()))